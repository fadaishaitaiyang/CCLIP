...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        ...
        lists = [0, 0,0, val_loss.item() / (iteration + 1)]
...
def compute_heatmap_loss_by_gtbox_predheatmap(box, heatmap):
    ...
    for i in range(batch_size):
        ...
        gt_channel = np.zeros((64, 64), dtype=np.float32)
        cur_box = box[i]
        gt_channel[int(cur_box[0] / 224 * 64):int(cur_box[2] / 224 * 64)+1, int(cur_box[1] / 224 * 64):int(cur_box[3] / 224 * 64)+1] = 1
        cur_heatmap = heatmap[i]
        gt_channel = torch.from_numpy(gt_channel).type(torch.FloatTensor).cuda()
        sorted_target, _ = torch.sort(gt_channel)
        sorted_pred, _ = torch.sort(cur_heatmap)
        w_distance = torch.mean(torch.abs(sorted_pred - sorted_target))
        
        lc = F.cross_entropy(cur_heatmap,gt_channel,None,reduction='none',ignore_index=-100)
        eng_loss =  lc + torch.log(w_distance + 1)
