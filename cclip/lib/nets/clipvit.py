...
class CLIPTextEncoder(nn.Module):
    def __init__(self, context_length=22,
        ...
        self.positional_embedding = nn.Parameter(torch.empty(82, transformer_width))
    ...
    def build_attention_mask(self):
        ...
        mask = torch.empty(82, 82)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # zero out the lower diagonal
        return mask

    def forward(self, text, context):
        # print(text.shape)[25, 13]
        x_text = self.token_embedding(text)  # n_clas, n_text, C||context(1,5,512) text(25,77)
        # print(x_text.shape) [25, 13, 512]
        K, N1, C = x_text.shape # x_text(25,77,1024)
        B, N2, C = context.shape
        # print(context.shape) [1, 5, 512]
        eos_indx = text.argmax(dim=-1) + N2 #eos_indx(25)
        # print(eos_indx.shape) [25]
        eos_indx = eos_indx.reshape(1, K).expand(B, K).reshape(-1)#eos_indx(25)
        # print(eos_indx.shape) [25]
        x_text = x_text.reshape(1, K, N1, C).expand(B, K, N1, C) #x_text(1,25,77,1024)
        # print(x_text.shape) [1, 25, 13, 512]
        context = context.reshape(B, 1, N2, C).expand(B, K, N2, C) # context(1,25,5,1024)
        # print(context.shape) [1, 25, 5, 512]
        x = torch.cat([x_text[:,:,0:1], context, x_text[:, :, 1:]], dim=2).reshape(B*K, N1+N2, C)#x(25,82,1024)
        # print(x.shape) [25, 18, 512]
        
        # print(x.shape) #[25, 18, 512]
        # print(self.positional_embedding.shape) # [13, 512]
        x = x + self.positional_embedding #x(25,82,1024)
        # print(x.shape) #(25,18,512)
        x = x.permute(1, 0, 2)  # NLD -> LND #x(82,25,512)
        # print(x.shape) #[18, 25, 512]
        x = self.transformer(x)#x(82,25,1024)
        # print(x.shape) # [18, 25, 512]
        x = x.permute(1, 0, 2)  # LND -> NLD#x(25,82,1024)
        # print(x.shape) #[25, 18, 512]
        x = self.ln_final(x) #x(25,82,1024)
        # print(x.shape) #[25, 18, 512]
        x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection#x(25,512)
        # print(x.shape) #[25, 512]
        x = x.reshape(B, K, self.embed_dim) #x(1,25,512)
        # print(x.shape) #[1, 25, 512]
        return x
...
#         visual  = visual.reshape(B,C,49).permute(0,2,1)
#         B, N, C = visual.shape
